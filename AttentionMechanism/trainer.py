...
class Trainer():
    ...
    def train_normal(self, config):
        ...
        x_verb = np.array([dd[7] for d in data for dd in d])  # change for verb
        x_noun = np.array([dd[8] for d in data for dd in d])  # change for noun
        y_label = [dd[3] for d in data for dd in d]
        (x_target, x_word, x_entity, x_arg, split, x_quant, x_verb, x_noun), y_label = self.data_filter(
            (x_target, x_word, x_entity, x_arg, split, x_quant, x_verb, x_noun),
            y_label,
            config.remove_list
        )
        ...
        self.training_data = self.pack_data((x_target, x_word, x_entity, x_arg, x_quant, x_verb, x_noun, y_weight, y),
                                            index_list=(split == 0))
        self.eval_data = self.pack_data((x_target, x_word, x_entity, x_quant, x_verb, x_noun, y),
                                        index_list=(split == 1))
        self.testing_data = self.pack_data((x_target, x_word, x_entity, x_quant, x_verb, x_noun, y),
                                           index_list=(split == 2))
        ...
        with tf.device(config.machine):
            input_context_words = tf.placeholder(tf.int32, [None, config.context_window_size * 2],
                                                 name="input_context_words")
            ...
            input_verb = tf.placeholder(tf.int32, [None, config.context_window_size * 2], name="verb_types")
            input_noun = tf.placeholder(tf.int32, [None, config.context_window_size * 2], name="noun_types")
        ...
        for e in range(1, config.epochs + 1):
            ...
            o_ans_list = []
            # training
            for i, (x_target, x_word, x_entity, x_alpha, x_quant, x_verb, x_noun, y_weight, y) in enumerate(
                    # change for quant
                ...
                l, _, a, o_ans, p_ans = sess.run(
                    ...
                    feed_dict={
                        ...
                        input_verb: x_verb,  # change for verb
                        input_noun: x_noun,  # change for noun
                ...
                print(
                    "\rEpoch: {:>4}, {:>7} / {:>7} [{:>2.2f}%], Loss:{:>2.3f}, Acc:{:>2.2f}, Prec:{:>2.2f}, Rec:{:>2.2f}, F1:{:>2.2f}".format(
                        e,
                        i * config.batch_size,
                        config.train_size,
            ...
            p_ans_list = []
            o_ans_list = []
            for i, (x_target, x_word, x_entity, x_quant, x_verb, x_noun, y) in enumerate(
            ...
            if e % 10 == 0:
                if e % 100 == 0:
                    saver.save(sess, '/Users/natashamittal/Downloads/results/attention.model.upsampling', global_step=e)
                ...
                with open("/Users/natashamittal/Downloads/results/testing_upsampling_{}.res".format(e), 'w',
